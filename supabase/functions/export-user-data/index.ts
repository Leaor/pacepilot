...
const exportTables = [
  ...
  {
    key: "legacy_privacy_preferences",
  ...
  {
    key: "data_source_consents",
    table: "data_source_consents",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "activity_import_logs",
    table: "activity_import_logs",
    userColumn: "user_id",
    optional: true,
  },
  ...
  {
    key: "race_readiness_scores",
    table: "race_readiness_scores",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "race_checklists",
    table: "race_checklists",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "life_mode_requests",
    table: "life_mode_requests",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "weekly_adjustments",
    table: "weekly_adjustments",
    userColumn: "user_id",
    optional: true,
  },
  {
  ...
  {
    key: "legacy_ai_conversations",
    table: "ai_conversations",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "legacy_ai_messages",
    table: "ai_messages",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "ai_analysis_reports",
  ...
  {
    key: "ai_data_access_logs",
    table: "ai_data_access_logs",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "strava_connections",
  ...
  {
    key: "strava_activity_cache",
    table: "strava_activity_cache",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "garmin_connections",
  ...
  {
    key: "connected_service_audit_logs",
    table: "connected_service_audit_logs",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "user_data_export_requests",
    table: "user_data_export_requests",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "user_data_deletion_requests",
    table: "user_data_deletion_requests",
    userColumn: "user_id",
    optional: true,
  },
  {
    key: "legacy_account_deletion_requests",
...
function redactConnectionSecrets(rows: unknown): unknown {
  ...
  return rows.map((row) => {
    ...
    const { access_token_encrypted, refresh_token_encrypted, ...safeRow } =
...
async function recordExportRequest(
  ...
  const result = await supabase.from("user_data_export_requests").insert(
    buildExportRequestPayload(userId, completedAt),
  );
  if (!result.error) {
    return;
  }
  ...
  const legacyResult = await supabase
    ...
    .insert(buildLegacyExportRequestPayload(userId, completedAt));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await recordExportRequest(supabase, user.id, exportedAt);
    ...
    return safeErrorResponse(error, "Export failed", req);
  }
});
