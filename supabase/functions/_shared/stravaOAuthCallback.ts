...
import { encryptSecret } from "./encryption.ts";
import { verifyOAuthState } from "./oauthState.ts";
import { writeConnectedServiceAuditLog } from "./audit.ts";

export async function handleStravaOAuthCallback(req: Request): Promise<Response> {
  ...
  try {
    ...
    const athleteId = String(tokenPayload.athlete?.id ?? "");
    ...
    return jsonResponse({ connected: true }, 200, req);
  } catch (error) {
    return safeErrorResponse(error, "Strava OAuth callback failed", req);
  }
}
